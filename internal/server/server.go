...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/habits", s.list)
	s.mux.HandleFunc("POST /api/habits", s.create)
	s.mux.HandleFunc("GET /api/habits/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/habits/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-trailhead", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"habits": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Habit{} }
	wj(w, map[string]any{"habits": items, "count": len(items)})
}

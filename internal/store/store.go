...
// Habit is the primary tracked resource. Streak/BestStreak/TotalChecks/
// CheckedToday are computed by hydrateHabit at read time, not stored.
type Habit struct {
	ID           string `json:"id"`
...
// CheckIn records that a habit was completed on a particular date.
// (habit_id, date) is unique — a habit can only be checked once per day.
type CheckIn struct {
	ID        string `json:"id"`
...
// hydrateHabit fills in computed fields (streak, best_streak, total_checks,
// checked_today) on a habit loaded from disk.
...
// calcStreak walks back from today counting consecutive days with check-ins.
// Today not being checked yet is allowed — the streak starts from yesterday
// in that case so "I haven't checked in today yet" doesn't kill the display.
...
// calcBestStreak finds the longest run of consecutive check-in days.
...
func (d *DB) CreateHabit(h *Habit) error {
	...
	_, err := d.db.Exec(
		`INSERT INTO habits(id, name, description, frequency, color, created_at)
		 VALUES(?, ?, ?, ?, ?, ?)`,
...
func (d *DB) GetHabit(id string) *Habit {
	...
	var archived int
	err := d.db.QueryRow(
		`SELECT id, name, description, frequency, color, archived, created_at FROM habits WHERE id=?`,
		id,
	).Scan(&h.ID, &h.Name, &h.Description, &h.Frequency, &h.Color, &archived, &h.CreatedAt)
	if err != nil {
		return nil
	}
...
func (d *DB) UpdateHabit(id string, h *Habit) error {
	...
	_, err := d.db.Exec(
		`UPDATE habits SET name=?, description=?, frequency=?, color=?, archived=? WHERE id=?`,
		h.Name, h.Description, h.Frequency, h.Color, archived, id,
	)
	return err
}

// DeleteHabit removes the habit and all its check-ins. Note: SQLite ON DELETE
// CASCADE only fires if PRAGMA foreign_keys is on, which it isn't by default,
// so we explicitly delete check-ins first.
...
func (d *DB) CheckIn(habitID, date, note string) error {
	...
	// INSERT OR REPLACE because (habit_id, date) is unique — re-checking the
	// same day overwrites the note rather than failing.
	_, err := d.db.Exec(
		`INSERT OR REPLACE INTO checkins(id, habit_id, date, note, created_at)
		 VALUES(?, ?, ?, ?, ?)`,
...
func (d *DB) ListCheckIns(habitID string, days int) []CheckIn {
	...
	rows, _ := d.db.Query(
		`SELECT id, habit_id, date, note, created_at FROM checkins
		 WHERE habit_id=? AND date>=? ORDER BY date DESC`,
		habitID, since,
	)
	if rows == nil {
		return nil
	...
	for rows.Next() {
		var c CheckIn
		rows.Scan(&c.ID, &c.HabitID, &c.Date, &c.Note, &c.CreatedAt)
		out = append(out, c)
	}
	return out
}
...
// DayView bundles all today's habits with their check status for the
// dashboard's main view.
type DayView struct {
